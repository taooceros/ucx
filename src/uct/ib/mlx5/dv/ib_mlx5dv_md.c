...
uct_ib_mlx5_devx_reg_ksm(uct_ib_mlx5_md_t *md, uint64_t address, size_t length,
  ...
  UCT_IB_MLX5DV_SET(create_mkey_in, in, translations_octword_actual_size,
                    list_size);

  mr = UCS_PROFILE_NAMED_CALL_ALWAYS(
      "devx_create_mkey", mlx5dv_devx_obj_create, md->super.dev.ibv_context, in,
      uct_ib_mlx5_calc_mkey_inlen(list_size), out, sizeof(out));
  if (mr == NULL) {
  ...
  *mr_p = mr;
  *mkey =
      (UCT_IB_MLX5DV_GET(create_mkey_out, out, mkey_index) << 8) | md->mkey_tag;
...
static void uct_ib_mlx5_devx_ksm_log(uct_ib_mlx5_md_t *md, void *address,
  ...
  ucs_debug("%s: KSM%s %s memory registration status \"%s\" "
            ...
            UCS_PTR_BYTE_OFFSET(address, length), iova, atomic ? " atomic" : "",
            mkey_index);
}
 ...
 */
static ucs_status_t uct_ib_mlx5_devx_reg_ksm_data_mt(
    uct_ib_mlx5_md_t *md, void *address, uint64_t iova, int atomic,
    uint32_t mkey_index, const char *reason,
    uct_ib_mlx5_devx_ksm_data_t *ksm_data, struct mlx5dv_devx_obj **mr_p,
    uint32_t *mkey) {
  ...
  uct_ib_mlx5_devx_ksm_list_log(md, address, ksm_data->length, iova, 1, reason);
  ucs_log_indent(+1);
  klm = UCT_IB_MLX5DV_ADDR_OF(create_mkey_in, in, klm_pas_mtt);
  ucs_carray_for_each(mr, ksm_data->mrs, ksm_data->mr_num) {
    uct_ib_mlx5_devx_klm_entry_set(&klm, mr - ksm_data->mrs, mr_address, *mr);
  ...
  status = uct_ib_mlx5_devx_reg_ksm(
      md, iova - padding, ksm_data->length + padding, atomic, mkey_index,
      reason, list_size, chunk_size, in, mr_p, mkey);
  ucs_free(in);
...
static ucs_status_t uct_ib_mlx5_devx_reg_ksm_data_addr(
  ...
  for (i = 0; i < list_size; i++) {
    uct_ib_mlx5_devx_klm_entry_set(
        &klm, i, UCS_PTR_BYTE_OFFSET(address, i * UCT_IB_MD_MAX_MR_SIZE), mr);
  }
  ucs_log_indent(-1);
...
static ucs_status_t uct_ib_mlx5_devx_reg_ksm_data_contig(
  ...
  /* Add offset to workaround CREATE_MKEY range check issue */
  list_size =
      ucs_div_round_up(ksm_length + ucs_get_page_size(), UCT_IB_MD_MAX_MR_SIZE);

  return uct_ib_mlx5_devx_reg_ksm_data_addr(
      md, (void *)ksm_address, ksm_length, ksm_iova, atomic, mkey_index, reason,
      mr->super.ib, list_size, mr_p, mkey);
}
...
static void uct_ib_mlx5_devx_md_mr_lru_pop(uct_ib_mlx5_md_t *md,
  ...
  head = ucs_list_extract_head(&md->lru_rkeys.list, uct_ib_mlx5_mem_lru_entry_t,
                               list);
 ...
 * @return Error code.
 */
static ucs_status_t uct_ib_md_mlx5_devx_mr_lru_push(uct_ib_mlx5_md_t *md,
                                                    uint32_t rkey, void *mr) {
  uct_ib_mlx5_mem_lru_entry_t *entry;
  khint_t iter;
  ucs_kh_put_t res;
  ...
  if (res == UCS_KH_PUT_KEY_PRESENT) {
    ucs_trace("%s: mr lru size=%d reset: %x->%p",
              uct_ib_device_name(&md->super.dev), kh_size(&md->lru_rkeys.hash),
              rkey, mr);
    entry = kh_val(&md->lru_rkeys.hash, iter);
    ucs_assertv(entry->rkey == rkey, "entry_rkey=0x%x, rkey=0x%x", entry->rkey,
                rkey);
  ...
  if (mr == NULL) {
    ucs_trace("%s: mr lru size=%d miss: %x", uct_ib_device_name(&md->super.dev),
  ...
  ucs_trace("%s: mr lru size=%d push: %x->%p",
            uct_ib_device_name(&md->super.dev), kh_size(&md->lru_rkeys.hash),
            rkey, mr);
...
/*
 * Register KSM data for given memory handle. Distinguish the way of KSM
 * creation structures filling by checking UCT_IB_MEM_MULTITHREADED flag.
 */
static ucs_status_t uct_ib_mlx5_devx_reg_ksm_data(
    uct_ib_mlx5_md_t *md, uct_ib_mlx5_devx_mem_t *memh,
    uct_ib_mr_type_t mr_type, uint32_t iova_offset, int atomic,
    uint32_t mkey_index, const char *reason, struct mlx5dv_devx_obj **mr_p,
    uint32_t *mkey) {
  ...
  if (memh->super.flags & UCT_IB_MEM_MULTITHREADED) {
    return uct_ib_mlx5_devx_reg_ksm_data_mt(md, address, iova, atomic,
                                            mkey_index, reason, mr->ksm_data,
                                            mr_p, mkey);
  } else {
    return uct_ib_mlx5_devx_reg_ksm_data_contig(
        md, address, iova, atomic, mkey_index, reason, mr, mr_p, mkey);
  }
}
...
UCS_PROFILE_FUNC_ALWAYS(ucs_status_t, uct_ib_mlx5_devx_reg_indirect_key,
  ...
  if (status != UCS_OK) {
    ucs_error("%s: LRU push returned %s", uct_ib_device_name(&md->super.dev),
...
uct_ib_mlx5_devx_reg_mt(uct_ib_mlx5_md_t *md, void *address, size_t length,
  ...
  ucs_trace("multithreaded register memory %p..%p chunks %d origin addr %p "
            ...
            prefix_address, UCS_PTR_BYTE_OFFSET(prefix_address, length), mr_num,
            address, md->super.dev.atomic_align, atomic_prefix);
  ...
  status =
      uct_ib_md_handle_mr_list_mt(&md->super, prefix_address, length, params,
                                  access_flags, mr_num, ksm_data->mrs);
  if (status != UCS_OK) {
    goto err_free;
  }

  status = uct_ib_mlx5_devx_reg_ksm_data_mt(
      md, prefix_address, (uint64_t)prefix_address, is_atomic, 0,
      "multi-thread-key", ksm_data, &ksm_data->dvmr, mkey_p);
  if (status != UCS_OK) {
    goto err_dereg;
  }
...
err_dereg:
  uct_ib_md_handle_mr_list_mt(&md->super, address, length, NULL, 0, mr_num,
                              ksm_data->mrs);
err_free:
  ucs_free(ksm_data);
err:
...
static void uct_ib_mlx5_devx_reg_symmetric(uct_ib_mlx5_md_t *md,
  ...
  while (md->smkey_index < md->super.mkey_by_name_reserve.size) {
    status = uct_ib_mlx5_devx_reg_ksm_data_contig(
        ...
        md->super.mkey_by_name_reserve.base + md->smkey_index, "symmetric-key",
        &memh->mrs[UCT_IB_MR_DEFAULT], &smkey_mr, &symmetric_rkey);
    if (status == UCS_OK) {
...
uct_ib_mlx5_devx_reg_mr(uct_ib_mlx5_md_t *md, uct_ib_mlx5_devx_mem_t *memh,
  ...
  uint64_t access_flags =
      uct_ib_memh_access_flags(&md->super, &memh->super) & access_mask;
  ...
  if ((length >= md->super.config.min_mt_reg) &&
    ...
    status = uct_ib_mlx5_devx_reg_mt(
        md, address, length,
        (memh->super.flags & UCT_IB_MEM_ACCESS_REMOTE_ATOMIC), params,
        access_flags, &mkey, &memh->mrs[mr_type].ksm_data);
    if (status == UCS_OK) {
      *rkey_p = *lkey_p = mkey;
...
static ucs_status_t
uct_ib_mlx5_devx_memh_alloc(uct_ib_mlx5_md_t *md, size_t length, unsigned flags,
                            size_t mr_size, uct_ib_mlx5_devx_mem_t **memh_p) {
  uct_ib_mlx5_devx_mem_t *memh;
  uct_ib_mem_t *ib_memh;
  ucs_status_t status;

  status = uct_ib_memh_alloc(&md->super, length, flags, sizeof(*memh), mr_size,
                             &ib_memh);
  if (status != UCS_OK) {
    return status;
  }
...
static ucs_status_t uct_ib_mlx5_devx_mem_reg_gva(uct_md_h uct_md,
                                                 uct_mem_h *memh_p) {
  ...
  if (md->super.relaxed_order) {
    status = uct_ib_reg_mr(&md->super, NULL, SIZE_MAX, &params,
                           access_flags & ~IBV_ACCESS_RELAXED_ORDERING, NULL,
...
ucs_status_t uct_ib_mlx5_devx_mem_reg(uct_md_h uct_md, void *address,
                                      size_t length,
                                      const uct_md_mem_reg_params_t *params,
                                      uct_mem_h *memh_p) {
  uct_ib_mlx5_md_t *md = ucs_derived_of(uct_md, uct_ib_mlx5_md_t);
  ...
  status = uct_ib_mlx5_devx_memh_alloc(md, length, flags, sizeof(memh->mrs[0]),
                                       &memh);
  if (status != UCS_OK) {
    goto err;
  }
  ...
  if (md->super.relaxed_order) {
    status = uct_ib_mlx5_devx_reg_mr(
        md, memh, address, length, params, UCT_IB_MR_STRICT_ORDER,
        ~IBV_ACCESS_RELAXED_ORDERING, &dummy_mkey, &dummy_mkey);
    if (status != UCS_OK) {
      goto err_dereg_default;
    }
  }
...
static ucs_status_t uct_ib_devx_dereg_invalidate_rkey_check(
    uct_ib_mlx5_md_t *md, uct_ib_mlx5_devx_mem_t *memh, uint32_t rkey,
    unsigned flags_mask, uint64_t cap_mask, const char *name) {
...
static ucs_status_t uct_ib_mlx5_devx_dereg_keys(uct_ib_mlx5_md_t *md,
                                                uct_ib_mlx5_devx_mem_t *memh) {
  ucs_status_t status;
  ...
  if (memh->atomic_dvmr != NULL) {
    ...
    status = uct_ib_mlx5_devx_obj_destroy(memh->atomic_dvmr, "MKEY, ATOMIC");
    if (status != UCS_OK) {
      return status;
    }
  }
  ...
  if (memh->indirect_dvmr != NULL) {
    ...
    ucs_trace("%s: destroy indirect_dvmr %p with key %x",
              ...
              memh->indirect_rkey);
    status =
        uct_ib_mlx5_devx_obj_destroy(memh->indirect_dvmr, "MKEY, INDIRECT");
    if (status != UCS_OK) {
      return status;
    }
  }
...
static ucs_status_t uct_ib_mlx5_devx_umr_create_qp(uct_ib_mlx5_md_t *md) {
  ...
  qp_attr_ex.comp_mask = IBV_QP_INIT_ATTR_SEND_OPS_FLAGS | IBV_QP_INIT_ATTR_PD;
  ...
  mlx5_qp_attr.send_ops_flags =
      MLX5DV_QP_EX_WITH_MR_LIST | MLX5DV_QP_EX_WITH_MR_INTERLEAVED;

  md->umr.qp =
      mlx5dv_create_qp(md->super.dev.ibv_context, &qp_attr_ex, &mlx5_qp_attr);
  if (NULL == md->umr.qp) {
...
static ucs_status_t uct_ib_mlx5_devx_umr_modify_qp(uct_ib_mlx5_md_t *md) {
  ...
  /* Modify to RTR */
  attr_mask = IBV_QP_STATE | IBV_QP_DEST_QPN | IBV_QP_PATH_MTU | IBV_QP_RQ_PSN |
              IBV_QP_MIN_RNR_TIMER | IBV_QP_MAX_DEST_RD_ATOMIC | IBV_QP_AV;
  ...
  if (UCS_OK !=
      uct_ib_device_query_gid(ibdev, port_num, UCT_IB_DEVICE_DEFAULT_GID_INDEX,
  ...
  /* Modify to RTS */
  attr_mask = IBV_QP_STATE | IBV_QP_SQ_PSN | IBV_QP_TIMEOUT | IBV_QP_RNR_RETRY |
              IBV_QP_RETRY_CNT | IBV_QP_MAX_QP_RD_ATOMIC;
...
static void uct_ib_mlx5_devx_umr_mkey_alias_destroy(
    uct_ib_mlx5_md_t *md, uct_ib_mlx5_devx_umr_alias_t *umr_alias) {
  ucs_status_t status;

  ucs_trace("%s: destroy " UCT_IB_MLX5_UMR_ALIAS_FMT, uct_ib_mlx5_dev_name(md),
...
uct_ib_mlx5_devx_umr_mkey_destroy(uct_ib_mlx5_md_t *md,
  ...
  if (0 != ret) {
    ucs_warn("%s: mlx5dv_destroy_mkey(" UCT_IB_MLX5_UMR_MKEY_FMT ") returned "
             "%d: %m",
             uct_ib_mlx5_dev_name(md), UCT_IB_MLX5_UMR_MKEY_ARG(umr_mkey), ret);
  }
...
uct_ib_mlx5_devx_umr_mkey_create(uct_ib_mlx5_md_t *md) {
  ...
  ucs_trace("%s: created " UCT_IB_MLX5_UMR_MKEY_FMT, uct_ib_mlx5_dev_name(md),
            UCT_IB_MLX5_UMR_MKEY_ARG(umr_mkey));
  return umr_mkey;
}
...
static void uct_ib_mlx5_devx_umr_cleanup(uct_ib_mlx5_md_t *md) {
  ...
  if (!ucs_list_is_empty(&md->umr.mkey_pool)) {
    ucs_trace("%s: destroy UMR mkey pool with %lu elements",
              uct_ib_mlx5_dev_name(md), ucs_list_length(&md->umr.mkey_pool));
...
static uct_ib_mlx5_devx_umr_mkey_t *
uct_ib_mlx5_devx_umr_mkey_bind(uct_ib_mlx5_md_t *md,
                               uct_ib_mlx5_devx_mr_t *mr) {
  struct ibv_mr *ib_mr = mr->super.ib;
  struct ibv_mw mw = {.rkey = 0, .type = IBV_MW_TYPE_1};
  ...
  ucs_trace("%s: registered addr %p length %zu lkey 0x%x "
            "to " UCT_IB_MLX5_UMR_MKEY_FMT,
            uct_ib_mlx5_dev_name(md), ib_mr->addr, ib_mr->length, ib_mr->lkey,
            UCT_IB_MLX5_UMR_MKEY_ARG(umr_mkey));
  return umr_mkey;
}

static UCS_F_ALWAYS_INLINE uint64_t uct_ib_mlx5_devx_umr_mkey_hash_code(
...
static ucs_status_t uct_ib_mlx5_devx_umr_mkey_hash_put(
    uct_ib_mlx5_md_t *md, const uct_ib_md_packed_mkey_t *packed_mkey,
...
uct_ib_mlx5_devx_mem_dereg(uct_md_h uct_md,
  ...
  if (memh->smkey_mr != NULL) {
    ucs_trace("%s: destroy smkey_mr %p with key %x",
              ...
              memh->super.rkey);
    status = uct_ib_mlx5_devx_obj_destroy(memh->smkey_mr, "MKEY, SYMMETRIC");
    if (status != UCS_OK) {
      return status;
    }
  }
  ...
  if (memh->exported_umr_mkey != NULL) {
    status = uct_ib_mlx5_devx_umr_mkey_invalidate(md, memh->exported_umr_mkey);
    if (status != UCS_OK) {
      return status;
    }
  }
  ...
  if (!(memh->super.flags & UCT_IB_MEM_IMPORTED)) {
    if (md->super.relaxed_order) {
      status = uct_ib_mlx5_devx_dereg_mr(md, memh, UCT_IB_MR_STRICT_ORDER);
      if (status != UCS_OK) {
        return status;
      }
    }
...
static ucs_status_t uct_ib_mlx5_add_page(ucs_mpool_t *mp, size_t *size_p,
                                         void **page_p) {
...
static void uct_ib_mlx5_devx_check_odp(uct_ib_mlx5_md_t *md,
  ...
  status =
      uct_ib_mlx5_devx_general_cmd(md->super.dev.ibv_context, in, sizeof(in),
                                   out, sizeof(out), "QUERY_HCA_CAP, ODP", 0);
  if (status != UCS_OK) {
  ...
  if (UCT_IB_MLX5DV_GET(query_hca_cap_out, out,
    ...
    odp_cap =
        UCT_IB_MLX5DV_ADDR_OF(query_hca_cap_out, out,
...
static ucs_status_t uct_ib_mlx5_devx_query_lag(uct_ib_mlx5_md_t *md,
                                               uint8_t *state) {
  char out[UCT_IB_MLX5DV_ST_SZ_BYTES(query_lag_out)] = {};
  char in[UCT_IB_MLX5DV_ST_SZ_BYTES(query_lag_in)] = {};
  void *lag;
...
uct_ib_mlx5_devx_open_device(struct ibv_device *ibv_device) {
  ...
  if (cq == NULL) {
    uct_ib_check_memlock_limit_msg(ctx, UCS_LOG_LEVEL_DEBUG, "ibv_create_cq()");
    goto close_ctx;
  }
...
static void uct_ib_mlx5_devx_init_flush_mr(uct_ib_mlx5_md_t *md) {
  ...
  status =
      uct_ib_reg_mr(&md->super, md->zero_buf, UCT_IB_MD_FLUSH_REMOTE_LENGTH,
                    &params, UCT_IB_MEM_ACCESS_FLAGS, NULL, &md->flush_mr);
  if (status != UCS_OK) {
    goto err;
  }

  status = uct_ib_mlx5_devx_reg_ksm_data_addr(
      md, md->zero_buf, UCT_IB_MD_FLUSH_REMOTE_LENGTH, 0, 0, 0, "flush-mr",
      md->flush_mr, 1, &md->flush_dvmr, &md->super.flush_rkey);
  if (status != UCS_OK) {
    goto err_dereg_mr;
  }
...
static ucs_status_t uct_ib_mlx5_devx_query_cap_2(struct ibv_context *ctx,
...
static void uct_ib_mlx5_devx_check_mkey_by_name(uct_ib_mlx5_md_t *md,
  ...
  log_size =
      UCT_IB_MLX5DV_GET(cmd_hca_cap_2, cap_2, mkey_by_name_reserve_log_size);
...
uct_ib_mlx5_devx_device_mem_alloc(uct_md_h uct_md, size_t *length_p,
   ...
   */
  address =
      mmap(NULL, dm_attr.length, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  ...
  status = uct_ib_mlx5_devx_reg_ksm_data_addr(
      ...
      memh->mrs[UCT_IB_MR_DEFAULT].super.ib, 1, &memh->dm_addr_dvmr, &mkey);
  if (status != UCS_OK) {
    goto err_dereg_dm;
  }
...
ucs_status_t uct_ib_mlx5_devx_device_mem_free(uct_md_h uct_md,
                                              uct_mem_h tl_memh) {
#if HAVE_IBV_DM
  uct_ib_mlx5_md_t *md = ucs_derived_of(uct_md, uct_ib_mlx5_md_t);
...
static void uct_ib_mlx5dv_check_dm_ksm_reg(uct_ib_mlx5_md_t *md) {
  ...
  status = uct_ib_mlx5_devx_device_mem_alloc(
      uct_md, &length, &address, UCS_MEMORY_TYPE_RDMA, 0,
      "check dm ksm registration", &memh);
  if (status != UCS_OK) {
...
ucs_status_t uct_ib_mlx5_devx_md_open(struct ibv_device *ibv_device,
  ...
  md->log_max_dci_stream_channels =
      UCT_IB_MLX5DV_GET(cmd_hca_cap, cap, log_max_dci_stream_channels);
  md->log_max_dci_stream_channels =
      ucs_min(md->log_max_dci_stream_channels, UCT_IB_MLX5_MD_MAX_DCI_CHANNELS);
  ...
  max_rd_atomic_dc =
      1 << UCT_IB_MLX5DV_GET(cmd_hca_cap, cap, log_max_ra_req_dc);
  ...
  if (UCT_IB_MLX5DV_GET(cmd_hca_cap, cap, ib_striding_wq_cq_first_indication)) {
  ...
  if (UCT_IB_MLX5DV_GET(cmd_hca_cap, cap, atomic)) {
    int ops =
        UCT_IB_MLX5_ATOMIC_OPS_CMP_SWAP | UCT_IB_MLX5_ATOMIC_OPS_FETCH_ADD;
    uint8_t arg_size;
    int cap_ops, mode8b;
    ...
    dev->pci_fadd_arg_sizes =
        UCT_IB_MLX5DV_GET(atomic_caps, cap, fetch_add_pci_atomic) << 2;
    dev->pci_cswap_arg_sizes =
        UCT_IB_MLX5DV_GET(atomic_caps, cap, compare_swap_pci_atomic) << 2;
  }
  ...
  ucs_debug("%s: opened DEVX md log_max_qp=%d", uct_ib_device_name(dev),
            log_max_qp);
  ...
  if (md->flags & UCT_IB_MLX5_MD_FLAG_KSM) {
    ...
    if (md->flags & UCT_IB_MLX5_MD_FLAG_INDIRECT_ATOMICS) {
      md->super.cap_flags |=
          UCT_MD_FLAG_INVALIDATE_AMO | UCT_MD_FLAG_INVALIDATE;
      ksm_atomic = 1;
    }
...
uint8_t uct_ib_mlx5_devx_md_get_counter_set_id(uct_ib_mlx5_md_t *md,
                                               uint8_t port_num) {
...
ucs_status_t uct_ib_mlx5_devx_allow_xgvmi_access(uct_ib_mlx5_md_t *md,
  ...
  UCT_IB_MLX5DV_SET(allow_other_vhca_access_in, in, object_type_to_be_accessed,
  ...
  access_key =
      UCT_IB_MLX5DV_ADDR_OF(allow_other_vhca_access_in, in, access_key);
  ucs_strncpy_zero(access_key, uct_ib_mkey_token,
                   UCT_IB_MLX5DV_FLD_SZ_BYTES(alias_context, access_key));

  return uct_ib_mlx5_devx_general_cmd(md->super.dev.ibv_context, in, sizeof(in),
                                      out, sizeof(out),
                                      "ALLOW_OTHER_VHCA_ACCESS", silent);
}

static ucs_status_t
uct_ib_mlx5_devx_xgvmi_umem_mr(uct_ib_mlx5_md_t *md,
                               uct_ib_mlx5_devx_mem_t *memh) {
  ...
  aligned_address = ucs_align_down_pow2_ptr(memh->address, ucs_get_page_size());
...
uct_ib_mlx5_devx_reg_xgvmi_ksm_mr(uct_ib_mlx5_md_t *md,
  ...
  status =
      uct_ib_mlx5_devx_reg_ksm_data(md, memh, UCT_IB_MR_DEFAULT, 0, 0, 0,
                                    "exported-key", &cross_mr, &exported_lkey);
  if (status != UCS_OK) {
    return status;
  }
...
ucs_status_t uct_ib_mlx5_devx_mkey_pack(uct_md_h uct_md, uct_mem_h uct_memh,
                                        void *address, size_t length,
  ...
  if (flags & UCT_MD_MKEY_PACK_FLAG_EXPORT) {
    ...
    uct_ib_md_pack_exported_mkey(&md->super, memh->exported_lkey, mkey_buffer);
    return UCS_OK;
  }
  ...
  if (ucs_unlikely(memh->atomic_dvmr == NULL) &&
      ...
      !(memh->super.flags & UCT_IB_MEM_IMPORTED) &&
      ucs_test_all_flags(md->flags, UCT_IB_MLX5_MD_FLAG_KSM |
...
ucs_status_t uct_ib_mlx5_devx_mem_attach(uct_md_h uct_md,
  ...
  umr_alias.cross_mr = uct_ib_mlx5_devx_obj_create(
      md->super.dev.ibv_context, in, sizeof(in), out, sizeof(out), "MKEY_ALIAS",
...
ucs_status_t uct_ib_mlx5_devx_md_query(uct_md_h uct_md,
    ...
    .super =
        {
            ...
            .mem_attach_verbs = uct_ib_mlx5_devx_mem_attach_ibverbs,
...
static void uct_ib_mlx5dv_check_dc(uct_ib_device_t *dev) {
  ...
  qp = UCS_PROFILE_CALL_ALWAYS(mlx5dv_create_qp, ctx, &qp_init_attr, &dv_attr);
  if (qp == NULL) {
    ucs_debug("%s: mlx5dv_create_qp(DCT) failed: %m", uct_ib_device_name(dev));
...
static ucs_status_t uct_ib_mlx5dv_md_open(struct ibv_device *ibv_device,
  ...
  if (ctx == NULL) {
    ucs_diag("ibv_open_device(%s) failed: %m", ibv_get_device_name(ibv_device));
    status = UCS_ERR_UNSUPPORTED;
    goto err;
  }
