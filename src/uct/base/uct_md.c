 ...
 * RESERVED. Copyright (C) UT-Battelle, LLC. 2015. ALL RIGHTS RESERVED.
...
#define UCT_MD_ATTR_V2_FIELD_COPY(_md_attr_dst, _md_attr_src, _field_name,     \
  ...
  {                                                                            \
    if ((_md_attr_dst)->field_mask & (_field_flag)) {                          \
      ...
      memcpy(&((_md_attr_dst)->_field_name), &((_md_attr_src)->_field_name),   \
...
uct_md_query_single_md_resource(uct_component_t *component,
  ...
  ucs_snprintf_zero(resource->md_name, UCT_MD_NAME_MAX, "%s", component->name);
...
ucs_status_t uct_config_modify(void *config, const char *name,
                               const char *value) {
  uct_config_bundle_t *bundle = (uct_config_bundle_t *)config - 1;
...
static ucs_status_t uct_md_mkey_pack_params_check(uct_md_h md, uct_mem_h memh,
                                                  void *mkey_buffer) {
...
ucs_status_t uct_md_mkey_pack_v2(uct_md_h md, uct_mem_h memh, void *address,
                                 size_t length,
...
ucs_status_t uct_rkey_compare(uct_component_h component, uct_rkey_t rkey1,
                              uct_rkey_t rkey2,
                              const uct_rkey_compare_params_t *params,
                              int *result) {
...
static void uct_md_attr_from_v2(uct_md_attr_t *dst,
                                const uct_md_attr_v2_t *src) {
...
static void uct_md_attr_v2_copy(uct_md_attr_v2_t *dst,
                                const uct_md_attr_v2_t *src) {
  ...
  UCT_MD_ATTR_V2_FIELD_COPY(dst, src, local_cpus, UCT_MD_ATTR_FIELD_LOCAL_CPUS);
  ...
  UCT_MD_ATTR_V2_FIELD_COPY(dst, src, global_id, UCT_MD_ATTR_FIELD_GLOBAL_ID);
  UCT_MD_ATTR_V2_FIELD_COPY(dst, src, reg_alignment,
                            UCT_MD_ATTR_FIELD_REG_ALIGNMENT);
}

static ucs_status_t uct_md_attr_v2_init(uct_md_h md,
  ...
  if (params->field_mask & UCT_MEM_ALLOC_PARAM_FIELD_FLAGS) {
    if (params->flags & UCT_MD_MEM_FLAG_FIXED) {
      ...
      if ((params->address == NULL) ||
        ...
        ucs_debug(
            "UCT_MD_MEM_FLAG_FIXED requires valid page size aligned address");
        return UCS_ERR_INVALID_PARAM;
      }
    }
  }
...
ucs_status_t uct_md_mem_dereg(uct_md_h md, uct_mem_h memh) {
  uct_md_mem_dereg_params_t params = {.field_mask = UCT_MD_MEM_DEREG_FIELD_MEMH,
                                      .memh = memh};
